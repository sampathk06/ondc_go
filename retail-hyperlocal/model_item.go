/*
ONDC Protocol API for retail (grocery, f&b)

ONDC Protocol API specification, which includes statutory requirements for packaged commodities and pre-packaged food This is an adaptation of Beckn Core version 0.9.3

API version: 1.0.27
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// Item Describes a product or a service offered to the end consumer by the provider - Statutory requirements for category_id \"Packaged Commodities\" - descriptor.name (for item name), price.value (for declared price), price.maximum_value (for MRP); Statutory requirements for category_id \"Packaged Foods\" - descriptor.name (for item name), descriptor.symbol (for veg or non-veg), descriptor.short_desc (for food allergen), descriptor.long_desc (for instructions for use), price.value (for declared price); All other statutory attributes must be provided, as applicable, for the above; No statutory requirement for category_id \"F&B\" or \"Fruits and Vegetables\"
type Item struct {
	// This is the most unique identifier of a service item. An example of an Item ID could be the SKU of a product.
	Id string `json:"id"`
	ParentItemId *Id `json:"parent_item_id,omitempty"`
	Descriptor Descriptor `json:"descriptor"`
	Price Price `json:"price"`
	Quantity *ItemQuantity `json:"quantity,omitempty"`
	CategoryId Id `json:"category_id"`
	FulfillmentId Id `json:"fulfillment_id"`
	Rating *Value `json:"rating,omitempty"`
	LocationId Id `json:"location_id"`
	Time *Time `json:"time,omitempty"`
	// If the entity can be rated or not
	Rateable *bool `json:"rateable,omitempty"`
	Matched bool `json:"matched"`
	Related *bool `json:"related,omitempty"`
	Recommended *bool `json:"recommended,omitempty"`
	// whether the item is returnable
	OndcOrgReturnable bool `json:"@ondc/org/returnable"`
	// in case of return, whether the item should be picked up by seller
	OndcOrgSellerPickupReturn bool `json:"@ondc/org/seller_pickup_return"`
	// return window for the item in ISO8601 durations format e.g. 'PT24H' indicates 24 hour return window. Mandatory if \"@ondc/org/returnable\" is \"true\"
	OndcOrgReturnWindow string `json:"@ondc/org/return_window"`
	// whether the item is cancellable
	OndcOrgCancellable bool `json:"@ondc/org/cancellable"`
	// time from order confirmation by which item ready to ship in ISO8601 durations format (e.g. 'PT30M' indicates item ready to ship in 30 mins). Mandatory for category_id \"F&B\"
	OndcOrgTimeToShip string `json:"@ondc/org/time_to_ship"`
	// whether the catalog item is available on COD
	OndcOrgAvailableOnCod bool `json:"@ondc/org/available_on_cod"`
	// contact details for consumer care
	OndcOrgContactDetailsConsumerCare string `json:"@ondc/org/contact_details_consumer_care"`
	OndcOrgStatutoryReqsPackagedCommodities ItemOndcOrgStatutoryReqsPackagedCommodities `json:"@ondc/org/statutory_reqs_packaged_commodities"`
	OndcOrgStatutoryReqsPrepackagedFood ItemOndcOrgStatutoryReqsPrepackagedFood `json:"@ondc/org/statutory_reqs_prepackaged_food"`
	// mandatory for category_id \"Fruits and Vegetables\" required attributes include the following - net_quantity
	OndcOrgMandatoryReqsVeggiesFruits map[string]interface{} `json:"@ondc/org/mandatory_reqs_veggies_fruits"`
	// Describes a tag. This is a simple key-value store which is used to contain extended metadata
	Tags *map[string]string `json:"tags,omitempty"`
}

// NewItem instantiates a new Item object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewItem(id string, descriptor Descriptor, price Price, categoryId Id, fulfillmentId Id, locationId Id, matched bool, ondcOrgReturnable bool, ondcOrgSellerPickupReturn bool, ondcOrgReturnWindow string, ondcOrgCancellable bool, ondcOrgTimeToShip string, ondcOrgAvailableOnCod bool, ondcOrgContactDetailsConsumerCare string, ondcOrgStatutoryReqsPackagedCommodities ItemOndcOrgStatutoryReqsPackagedCommodities, ondcOrgStatutoryReqsPrepackagedFood ItemOndcOrgStatutoryReqsPrepackagedFood, ondcOrgMandatoryReqsVeggiesFruits map[string]interface{}) *Item {
	this := Item{}
	this.Id = id
	this.Descriptor = descriptor
	this.Price = price
	this.CategoryId = categoryId
	this.FulfillmentId = fulfillmentId
	this.LocationId = locationId
	this.Matched = matched
	this.OndcOrgReturnable = ondcOrgReturnable
	this.OndcOrgSellerPickupReturn = ondcOrgSellerPickupReturn
	this.OndcOrgReturnWindow = ondcOrgReturnWindow
	this.OndcOrgCancellable = ondcOrgCancellable
	this.OndcOrgTimeToShip = ondcOrgTimeToShip
	this.OndcOrgAvailableOnCod = ondcOrgAvailableOnCod
	this.OndcOrgContactDetailsConsumerCare = ondcOrgContactDetailsConsumerCare
	this.OndcOrgStatutoryReqsPackagedCommodities = ondcOrgStatutoryReqsPackagedCommodities
	this.OndcOrgStatutoryReqsPrepackagedFood = ondcOrgStatutoryReqsPrepackagedFood
	this.OndcOrgMandatoryReqsVeggiesFruits = ondcOrgMandatoryReqsVeggiesFruits
	return &this
}

// NewItemWithDefaults instantiates a new Item object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewItemWithDefaults() *Item {
	this := Item{}
	return &this
}

// GetId returns the Id field value
func (o *Item) GetId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Id
}

// GetIdOk returns a tuple with the Id field value
// and a boolean to check if the value has been set.
func (o *Item) GetIdOk() (*string, bool) {
	if o == nil {
    return nil, false
	}
	return &o.Id, true
}

// SetId sets field value
func (o *Item) SetId(v string) {
	o.Id = v
}

// GetParentItemId returns the ParentItemId field value if set, zero value otherwise.
func (o *Item) GetParentItemId() Id {
	if o == nil || isNil(o.ParentItemId) {
		var ret Id
		return ret
	}
	return *o.ParentItemId
}

// GetParentItemIdOk returns a tuple with the ParentItemId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *Item) GetParentItemIdOk() (*Id, bool) {
	if o == nil || isNil(o.ParentItemId) {
    return nil, false
	}
	return o.ParentItemId, true
}

// HasParentItemId returns a boolean if a field has been set.
func (o *Item) HasParentItemId() bool {
	if o != nil && !isNil(o.ParentItemId) {
		return true
	}

	return false
}

// SetParentItemId gets a reference to the given Id and assigns it to the ParentItemId field.
func (o *Item) SetParentItemId(v Id) {
	o.ParentItemId = &v
}

// GetDescriptor returns the Descriptor field value
func (o *Item) GetDescriptor() Descriptor {
	if o == nil {
		var ret Descriptor
		return ret
	}

	return o.Descriptor
}

// GetDescriptorOk returns a tuple with the Descriptor field value
// and a boolean to check if the value has been set.
func (o *Item) GetDescriptorOk() (*Descriptor, bool) {
	if o == nil {
    return nil, false
	}
	return &o.Descriptor, true
}

// SetDescriptor sets field value
func (o *Item) SetDescriptor(v Descriptor) {
	o.Descriptor = v
}

// GetPrice returns the Price field value
func (o *Item) GetPrice() Price {
	if o == nil {
		var ret Price
		return ret
	}

	return o.Price
}

// GetPriceOk returns a tuple with the Price field value
// and a boolean to check if the value has been set.
func (o *Item) GetPriceOk() (*Price, bool) {
	if o == nil {
    return nil, false
	}
	return &o.Price, true
}

// SetPrice sets field value
func (o *Item) SetPrice(v Price) {
	o.Price = v
}

// GetQuantity returns the Quantity field value if set, zero value otherwise.
func (o *Item) GetQuantity() ItemQuantity {
	if o == nil || isNil(o.Quantity) {
		var ret ItemQuantity
		return ret
	}
	return *o.Quantity
}

// GetQuantityOk returns a tuple with the Quantity field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *Item) GetQuantityOk() (*ItemQuantity, bool) {
	if o == nil || isNil(o.Quantity) {
    return nil, false
	}
	return o.Quantity, true
}

// HasQuantity returns a boolean if a field has been set.
func (o *Item) HasQuantity() bool {
	if o != nil && !isNil(o.Quantity) {
		return true
	}

	return false
}

// SetQuantity gets a reference to the given ItemQuantity and assigns it to the Quantity field.
func (o *Item) SetQuantity(v ItemQuantity) {
	o.Quantity = &v
}

// GetCategoryId returns the CategoryId field value
func (o *Item) GetCategoryId() Id {
	if o == nil {
		var ret Id
		return ret
	}

	return o.CategoryId
}

// GetCategoryIdOk returns a tuple with the CategoryId field value
// and a boolean to check if the value has been set.
func (o *Item) GetCategoryIdOk() (*Id, bool) {
	if o == nil {
    return nil, false
	}
	return &o.CategoryId, true
}

// SetCategoryId sets field value
func (o *Item) SetCategoryId(v Id) {
	o.CategoryId = v
}

// GetFulfillmentId returns the FulfillmentId field value
func (o *Item) GetFulfillmentId() Id {
	if o == nil {
		var ret Id
		return ret
	}

	return o.FulfillmentId
}

// GetFulfillmentIdOk returns a tuple with the FulfillmentId field value
// and a boolean to check if the value has been set.
func (o *Item) GetFulfillmentIdOk() (*Id, bool) {
	if o == nil {
    return nil, false
	}
	return &o.FulfillmentId, true
}

// SetFulfillmentId sets field value
func (o *Item) SetFulfillmentId(v Id) {
	o.FulfillmentId = v
}

// GetRating returns the Rating field value if set, zero value otherwise.
func (o *Item) GetRating() Value {
	if o == nil || isNil(o.Rating) {
		var ret Value
		return ret
	}
	return *o.Rating
}

// GetRatingOk returns a tuple with the Rating field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *Item) GetRatingOk() (*Value, bool) {
	if o == nil || isNil(o.Rating) {
    return nil, false
	}
	return o.Rating, true
}

// HasRating returns a boolean if a field has been set.
func (o *Item) HasRating() bool {
	if o != nil && !isNil(o.Rating) {
		return true
	}

	return false
}

// SetRating gets a reference to the given Value and assigns it to the Rating field.
func (o *Item) SetRating(v Value) {
	o.Rating = &v
}

// GetLocationId returns the LocationId field value
func (o *Item) GetLocationId() Id {
	if o == nil {
		var ret Id
		return ret
	}

	return o.LocationId
}

// GetLocationIdOk returns a tuple with the LocationId field value
// and a boolean to check if the value has been set.
func (o *Item) GetLocationIdOk() (*Id, bool) {
	if o == nil {
    return nil, false
	}
	return &o.LocationId, true
}

// SetLocationId sets field value
func (o *Item) SetLocationId(v Id) {
	o.LocationId = v
}

// GetTime returns the Time field value if set, zero value otherwise.
func (o *Item) GetTime() Time {
	if o == nil || isNil(o.Time) {
		var ret Time
		return ret
	}
	return *o.Time
}

// GetTimeOk returns a tuple with the Time field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *Item) GetTimeOk() (*Time, bool) {
	if o == nil || isNil(o.Time) {
    return nil, false
	}
	return o.Time, true
}

// HasTime returns a boolean if a field has been set.
func (o *Item) HasTime() bool {
	if o != nil && !isNil(o.Time) {
		return true
	}

	return false
}

// SetTime gets a reference to the given Time and assigns it to the Time field.
func (o *Item) SetTime(v Time) {
	o.Time = &v
}

// GetRateable returns the Rateable field value if set, zero value otherwise.
func (o *Item) GetRateable() bool {
	if o == nil || isNil(o.Rateable) {
		var ret bool
		return ret
	}
	return *o.Rateable
}

// GetRateableOk returns a tuple with the Rateable field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *Item) GetRateableOk() (*bool, bool) {
	if o == nil || isNil(o.Rateable) {
    return nil, false
	}
	return o.Rateable, true
}

// HasRateable returns a boolean if a field has been set.
func (o *Item) HasRateable() bool {
	if o != nil && !isNil(o.Rateable) {
		return true
	}

	return false
}

// SetRateable gets a reference to the given bool and assigns it to the Rateable field.
func (o *Item) SetRateable(v bool) {
	o.Rateable = &v
}

// GetMatched returns the Matched field value
func (o *Item) GetMatched() bool {
	if o == nil {
		var ret bool
		return ret
	}

	return o.Matched
}

// GetMatchedOk returns a tuple with the Matched field value
// and a boolean to check if the value has been set.
func (o *Item) GetMatchedOk() (*bool, bool) {
	if o == nil {
    return nil, false
	}
	return &o.Matched, true
}

// SetMatched sets field value
func (o *Item) SetMatched(v bool) {
	o.Matched = v
}

// GetRelated returns the Related field value if set, zero value otherwise.
func (o *Item) GetRelated() bool {
	if o == nil || isNil(o.Related) {
		var ret bool
		return ret
	}
	return *o.Related
}

// GetRelatedOk returns a tuple with the Related field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *Item) GetRelatedOk() (*bool, bool) {
	if o == nil || isNil(o.Related) {
    return nil, false
	}
	return o.Related, true
}

// HasRelated returns a boolean if a field has been set.
func (o *Item) HasRelated() bool {
	if o != nil && !isNil(o.Related) {
		return true
	}

	return false
}

// SetRelated gets a reference to the given bool and assigns it to the Related field.
func (o *Item) SetRelated(v bool) {
	o.Related = &v
}

// GetRecommended returns the Recommended field value if set, zero value otherwise.
func (o *Item) GetRecommended() bool {
	if o == nil || isNil(o.Recommended) {
		var ret bool
		return ret
	}
	return *o.Recommended
}

// GetRecommendedOk returns a tuple with the Recommended field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *Item) GetRecommendedOk() (*bool, bool) {
	if o == nil || isNil(o.Recommended) {
    return nil, false
	}
	return o.Recommended, true
}

// HasRecommended returns a boolean if a field has been set.
func (o *Item) HasRecommended() bool {
	if o != nil && !isNil(o.Recommended) {
		return true
	}

	return false
}

// SetRecommended gets a reference to the given bool and assigns it to the Recommended field.
func (o *Item) SetRecommended(v bool) {
	o.Recommended = &v
}

// GetOndcOrgReturnable returns the OndcOrgReturnable field value
func (o *Item) GetOndcOrgReturnable() bool {
	if o == nil {
		var ret bool
		return ret
	}

	return o.OndcOrgReturnable
}

// GetOndcOrgReturnableOk returns a tuple with the OndcOrgReturnable field value
// and a boolean to check if the value has been set.
func (o *Item) GetOndcOrgReturnableOk() (*bool, bool) {
	if o == nil {
    return nil, false
	}
	return &o.OndcOrgReturnable, true
}

// SetOndcOrgReturnable sets field value
func (o *Item) SetOndcOrgReturnable(v bool) {
	o.OndcOrgReturnable = v
}

// GetOndcOrgSellerPickupReturn returns the OndcOrgSellerPickupReturn field value
func (o *Item) GetOndcOrgSellerPickupReturn() bool {
	if o == nil {
		var ret bool
		return ret
	}

	return o.OndcOrgSellerPickupReturn
}

// GetOndcOrgSellerPickupReturnOk returns a tuple with the OndcOrgSellerPickupReturn field value
// and a boolean to check if the value has been set.
func (o *Item) GetOndcOrgSellerPickupReturnOk() (*bool, bool) {
	if o == nil {
    return nil, false
	}
	return &o.OndcOrgSellerPickupReturn, true
}

// SetOndcOrgSellerPickupReturn sets field value
func (o *Item) SetOndcOrgSellerPickupReturn(v bool) {
	o.OndcOrgSellerPickupReturn = v
}

// GetOndcOrgReturnWindow returns the OndcOrgReturnWindow field value
func (o *Item) GetOndcOrgReturnWindow() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.OndcOrgReturnWindow
}

// GetOndcOrgReturnWindowOk returns a tuple with the OndcOrgReturnWindow field value
// and a boolean to check if the value has been set.
func (o *Item) GetOndcOrgReturnWindowOk() (*string, bool) {
	if o == nil {
    return nil, false
	}
	return &o.OndcOrgReturnWindow, true
}

// SetOndcOrgReturnWindow sets field value
func (o *Item) SetOndcOrgReturnWindow(v string) {
	o.OndcOrgReturnWindow = v
}

// GetOndcOrgCancellable returns the OndcOrgCancellable field value
func (o *Item) GetOndcOrgCancellable() bool {
	if o == nil {
		var ret bool
		return ret
	}

	return o.OndcOrgCancellable
}

// GetOndcOrgCancellableOk returns a tuple with the OndcOrgCancellable field value
// and a boolean to check if the value has been set.
func (o *Item) GetOndcOrgCancellableOk() (*bool, bool) {
	if o == nil {
    return nil, false
	}
	return &o.OndcOrgCancellable, true
}

// SetOndcOrgCancellable sets field value
func (o *Item) SetOndcOrgCancellable(v bool) {
	o.OndcOrgCancellable = v
}

// GetOndcOrgTimeToShip returns the OndcOrgTimeToShip field value
func (o *Item) GetOndcOrgTimeToShip() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.OndcOrgTimeToShip
}

// GetOndcOrgTimeToShipOk returns a tuple with the OndcOrgTimeToShip field value
// and a boolean to check if the value has been set.
func (o *Item) GetOndcOrgTimeToShipOk() (*string, bool) {
	if o == nil {
    return nil, false
	}
	return &o.OndcOrgTimeToShip, true
}

// SetOndcOrgTimeToShip sets field value
func (o *Item) SetOndcOrgTimeToShip(v string) {
	o.OndcOrgTimeToShip = v
}

// GetOndcOrgAvailableOnCod returns the OndcOrgAvailableOnCod field value
func (o *Item) GetOndcOrgAvailableOnCod() bool {
	if o == nil {
		var ret bool
		return ret
	}

	return o.OndcOrgAvailableOnCod
}

// GetOndcOrgAvailableOnCodOk returns a tuple with the OndcOrgAvailableOnCod field value
// and a boolean to check if the value has been set.
func (o *Item) GetOndcOrgAvailableOnCodOk() (*bool, bool) {
	if o == nil {
    return nil, false
	}
	return &o.OndcOrgAvailableOnCod, true
}

// SetOndcOrgAvailableOnCod sets field value
func (o *Item) SetOndcOrgAvailableOnCod(v bool) {
	o.OndcOrgAvailableOnCod = v
}

// GetOndcOrgContactDetailsConsumerCare returns the OndcOrgContactDetailsConsumerCare field value
func (o *Item) GetOndcOrgContactDetailsConsumerCare() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.OndcOrgContactDetailsConsumerCare
}

// GetOndcOrgContactDetailsConsumerCareOk returns a tuple with the OndcOrgContactDetailsConsumerCare field value
// and a boolean to check if the value has been set.
func (o *Item) GetOndcOrgContactDetailsConsumerCareOk() (*string, bool) {
	if o == nil {
    return nil, false
	}
	return &o.OndcOrgContactDetailsConsumerCare, true
}

// SetOndcOrgContactDetailsConsumerCare sets field value
func (o *Item) SetOndcOrgContactDetailsConsumerCare(v string) {
	o.OndcOrgContactDetailsConsumerCare = v
}

// GetOndcOrgStatutoryReqsPackagedCommodities returns the OndcOrgStatutoryReqsPackagedCommodities field value
func (o *Item) GetOndcOrgStatutoryReqsPackagedCommodities() ItemOndcOrgStatutoryReqsPackagedCommodities {
	if o == nil {
		var ret ItemOndcOrgStatutoryReqsPackagedCommodities
		return ret
	}

	return o.OndcOrgStatutoryReqsPackagedCommodities
}

// GetOndcOrgStatutoryReqsPackagedCommoditiesOk returns a tuple with the OndcOrgStatutoryReqsPackagedCommodities field value
// and a boolean to check if the value has been set.
func (o *Item) GetOndcOrgStatutoryReqsPackagedCommoditiesOk() (*ItemOndcOrgStatutoryReqsPackagedCommodities, bool) {
	if o == nil {
    return nil, false
	}
	return &o.OndcOrgStatutoryReqsPackagedCommodities, true
}

// SetOndcOrgStatutoryReqsPackagedCommodities sets field value
func (o *Item) SetOndcOrgStatutoryReqsPackagedCommodities(v ItemOndcOrgStatutoryReqsPackagedCommodities) {
	o.OndcOrgStatutoryReqsPackagedCommodities = v
}

// GetOndcOrgStatutoryReqsPrepackagedFood returns the OndcOrgStatutoryReqsPrepackagedFood field value
func (o *Item) GetOndcOrgStatutoryReqsPrepackagedFood() ItemOndcOrgStatutoryReqsPrepackagedFood {
	if o == nil {
		var ret ItemOndcOrgStatutoryReqsPrepackagedFood
		return ret
	}

	return o.OndcOrgStatutoryReqsPrepackagedFood
}

// GetOndcOrgStatutoryReqsPrepackagedFoodOk returns a tuple with the OndcOrgStatutoryReqsPrepackagedFood field value
// and a boolean to check if the value has been set.
func (o *Item) GetOndcOrgStatutoryReqsPrepackagedFoodOk() (*ItemOndcOrgStatutoryReqsPrepackagedFood, bool) {
	if o == nil {
    return nil, false
	}
	return &o.OndcOrgStatutoryReqsPrepackagedFood, true
}

// SetOndcOrgStatutoryReqsPrepackagedFood sets field value
func (o *Item) SetOndcOrgStatutoryReqsPrepackagedFood(v ItemOndcOrgStatutoryReqsPrepackagedFood) {
	o.OndcOrgStatutoryReqsPrepackagedFood = v
}

// GetOndcOrgMandatoryReqsVeggiesFruits returns the OndcOrgMandatoryReqsVeggiesFruits field value
func (o *Item) GetOndcOrgMandatoryReqsVeggiesFruits() map[string]interface{} {
	if o == nil {
		var ret map[string]interface{}
		return ret
	}

	return o.OndcOrgMandatoryReqsVeggiesFruits
}

// GetOndcOrgMandatoryReqsVeggiesFruitsOk returns a tuple with the OndcOrgMandatoryReqsVeggiesFruits field value
// and a boolean to check if the value has been set.
func (o *Item) GetOndcOrgMandatoryReqsVeggiesFruitsOk() (map[string]interface{}, bool) {
	if o == nil {
    return map[string]interface{}{}, false
	}
	return o.OndcOrgMandatoryReqsVeggiesFruits, true
}

// SetOndcOrgMandatoryReqsVeggiesFruits sets field value
func (o *Item) SetOndcOrgMandatoryReqsVeggiesFruits(v map[string]interface{}) {
	o.OndcOrgMandatoryReqsVeggiesFruits = v
}

// GetTags returns the Tags field value if set, zero value otherwise.
func (o *Item) GetTags() map[string]string {
	if o == nil || isNil(o.Tags) {
		var ret map[string]string
		return ret
	}
	return *o.Tags
}

// GetTagsOk returns a tuple with the Tags field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *Item) GetTagsOk() (*map[string]string, bool) {
	if o == nil || isNil(o.Tags) {
    return nil, false
	}
	return o.Tags, true
}

// HasTags returns a boolean if a field has been set.
func (o *Item) HasTags() bool {
	if o != nil && !isNil(o.Tags) {
		return true
	}

	return false
}

// SetTags gets a reference to the given map[string]string and assigns it to the Tags field.
func (o *Item) SetTags(v map[string]string) {
	o.Tags = &v
}

func (o Item) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if true {
		toSerialize["id"] = o.Id
	}
	if !isNil(o.ParentItemId) {
		toSerialize["parent_item_id"] = o.ParentItemId
	}
	if true {
		toSerialize["descriptor"] = o.Descriptor
	}
	if true {
		toSerialize["price"] = o.Price
	}
	if !isNil(o.Quantity) {
		toSerialize["quantity"] = o.Quantity
	}
	if true {
		toSerialize["category_id"] = o.CategoryId
	}
	if true {
		toSerialize["fulfillment_id"] = o.FulfillmentId
	}
	if !isNil(o.Rating) {
		toSerialize["rating"] = o.Rating
	}
	if true {
		toSerialize["location_id"] = o.LocationId
	}
	if !isNil(o.Time) {
		toSerialize["time"] = o.Time
	}
	if !isNil(o.Rateable) {
		toSerialize["rateable"] = o.Rateable
	}
	if true {
		toSerialize["matched"] = o.Matched
	}
	if !isNil(o.Related) {
		toSerialize["related"] = o.Related
	}
	if !isNil(o.Recommended) {
		toSerialize["recommended"] = o.Recommended
	}
	if true {
		toSerialize["@ondc/org/returnable"] = o.OndcOrgReturnable
	}
	if true {
		toSerialize["@ondc/org/seller_pickup_return"] = o.OndcOrgSellerPickupReturn
	}
	if true {
		toSerialize["@ondc/org/return_window"] = o.OndcOrgReturnWindow
	}
	if true {
		toSerialize["@ondc/org/cancellable"] = o.OndcOrgCancellable
	}
	if true {
		toSerialize["@ondc/org/time_to_ship"] = o.OndcOrgTimeToShip
	}
	if true {
		toSerialize["@ondc/org/available_on_cod"] = o.OndcOrgAvailableOnCod
	}
	if true {
		toSerialize["@ondc/org/contact_details_consumer_care"] = o.OndcOrgContactDetailsConsumerCare
	}
	if true {
		toSerialize["@ondc/org/statutory_reqs_packaged_commodities"] = o.OndcOrgStatutoryReqsPackagedCommodities
	}
	if true {
		toSerialize["@ondc/org/statutory_reqs_prepackaged_food"] = o.OndcOrgStatutoryReqsPrepackagedFood
	}
	if true {
		toSerialize["@ondc/org/mandatory_reqs_veggies_fruits"] = o.OndcOrgMandatoryReqsVeggiesFruits
	}
	if !isNil(o.Tags) {
		toSerialize["tags"] = o.Tags
	}
	return json.Marshal(toSerialize)
}

type NullableItem struct {
	value *Item
	isSet bool
}

func (v NullableItem) Get() *Item {
	return v.value
}

func (v *NullableItem) Set(val *Item) {
	v.value = val
	v.isSet = true
}

func (v NullableItem) IsSet() bool {
	return v.isSet
}

func (v *NullableItem) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableItem(val *Item) *NullableItem {
	return &NullableItem{value: val, isSet: true}
}

func (v NullableItem) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableItem) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


